...
public class ServletDelete2 extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for (int j=0;j<url.length;j++) {
            ...
            try {
                code[c++] = codeUtils.codeDelete(url[j],date.get(String.valueOf(j)));
            } catch (Exception e) {
                e.printStackTrace();
            }
            //获取body
            if (url[j]!= null) {
                try {
                    ...
                    s[a++] = utils.doDelete(url[j], date.get(String.valueOf(j)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                //获取响应头信息
                ...
                Header[] headers = null;
                try {
                    headers = headerUtils.doDelete(url[j], date.get(String.valueOf(j)));
            ...
            try {
                ...
                Info info1 = new Info(2,url[j],key[j],val[j],data,time,"delete",co);
