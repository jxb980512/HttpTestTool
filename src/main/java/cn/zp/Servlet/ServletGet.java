...
public class ServletGet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for (int j = 0; j < url.length; j++) {
            ...
            try {
                code[c++] = codeUtils.codeGet(url[j], date.get(String.valueOf(j)));
            } catch (Exception e) {
                e.printStackTrace();
            }
            //获取body
            if (url[j] != null) {
                try {
                    ...
                    s[a++] = utils.doGetHtml(url[j], date.get(String.valueOf(j)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                //获取响应头信息
                ...
                Header[] headers = null;
                try {
                    headers = headerUtils.doGetHtml(url[j], date.get(String.valueOf(j)));
                ...
                if(j<key.length){
                    ...
                    Info info1 = new Info(2,url[j],key[j],val[j],data,time,"get",co);
                    ...
                    try {
                        conn = JDBCUtils.getConnection3();
                        Info info1 = new Info(2,url[j],null,null,data,time,"get",co);
                        dao.insert(conn,info1);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    }finally {
